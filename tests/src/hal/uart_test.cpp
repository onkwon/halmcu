...
TEST(uart_driver, rx_interrupt_ShouldCallRxHandler) {
	...
	mock().expectOneCall("uart_get_event")
		.withParameter("port", PERIPH_UART0)
...
TEST(uart_driver, read_ShouldReturnZero_WhenNoDataReceived) {
	uint8_t c;
	uart_init(PERIPH_UART0, &default_cfg, &default_handle);
	mock().expectOneCall("uart_has_rx")
		.withParameter("port", PERIPH_UART0)
...
TEST(uart_driver, read_ShouldReturnReceivedData) {
	uint8_t buf[3];
	uart_init(PERIPH_UART0, &default_cfg, &default_handle);
	mock().expectNCalls(3, "uart_has_rx")
		.withParameter("port", PERIPH_UART0)
		.andReturnValue(true);
	mock().expectNCalls(3, "uart_get_rxd")
		.withParameter("port", PERIPH_UART0)
		.andReturnValue(1);
...
TEST(uart_driver, read_ShouldStopReading_WhenBufsizeShort) {
	uint8_t buf[1];
	uart_init(PERIPH_UART0, &default_cfg, &default_handle);
	mock().expectNCalls(1, "uart_has_rx")
		.withParameter("port", PERIPH_UART0)
		.andReturnValue(true);
	mock().expectNCalls(1, "uart_get_rxd")
		.withParameter("port", PERIPH_UART0)
		.andReturnValue(1);
