...
TEST(Watchdog, set_clock_source_ShouldSetPmuPcsr) {
	PMU->PCSR = 1;
	WDT->CON = 0;
	wdt_ll_set_clock_source(CLK_PLL);
	LONGS_EQUAL(0, PMU->PCSR);
	LONGS_EQUAL(8, WDT->CON);
	wdt_ll_set_clock_source(CLK_HSI);
	LONGS_EQUAL(1, PMU->PCSR);
	wdt_ll_set_clock_source(CLK_LSE);
	LONGS_EQUAL(2, PMU->PCSR);
	wdt_ll_set_clock_source(CLK_LSI);
	LONGS_EQUAL(3, PMU->PCSR);
}
TEST(Watchdog, set_clock_source_ShouldIgnoreParam_WhenInvalidGiven) {
	PMU->PCSR = 0x1000;
	wdt_ll_set_clock_source(CLK_PLL_BYPASS);
	LONGS_EQUAL(0x1000, PMU->PCSR);
}

TEST(Watchdog, get_clock_source_ShouldReturnClockSource) {
	PMU->PCSR = 1;
	LONGS_EQUAL(CLK_HSI, wdt_ll_get_clock_source());
	PMU->PCSR = 2;
	LONGS_EQUAL(CLK_LSE, wdt_ll_get_clock_source());
	PMU->PCSR = 3;
	LONGS_EQUAL(CLK_LSI, wdt_ll_get_clock_source());
	PMU->PCSR = 0;
	LONGS_EQUAL(CLK_PLL, wdt_ll_get_clock_source());
}
...
TEST(Watchdog, set_reload_ms_ShouldSetPrescalerAndReload) {
	wdt_ll_set_reload_ms(1000);
	LONGS_EQUAL(7, WDT->CON & 0x7);
	LONGS_EQUAL(3000, WDT->LR);
	wdt_ll_set_reload_ms(30000);
	LONGS_EQUAL(7, WDT->CON & 0x7);
	LONGS_EQUAL(90000, WDT->LR);
	wdt_ll_set_reload_ms(1);
	LONGS_EQUAL(7, WDT->CON & 0x7);
	LONGS_EQUAL(3, WDT->LR);
	wdt_ll_set_reload_ms(0);
