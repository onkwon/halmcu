...
TEST(I2C, write_byte_ShouldWriteToDrRegister) {
	i2c_write_byte(PERI_I2C0, 0x5a);
	LONGS_EQUAL(0x5a, I2C0->DR);
}

TEST(I2C, read_byte_ShouldReturnDrValue) {
	I2C0->DR = 0xa5;
	LONGS_EQUAL(0xa5, i2c_read_byte(PERI_I2C0));
}

TEST(I2C, clear_event_ShouldWriteFfToSrRegister) {
	i2c_clear_event(PERI_I2C0, I2C_EVENT_MASK);
	LONGS_EQUAL(0xff, I2C0->SR);
}

TEST(I2C, get_event_ShouldReturnEvents) {
	I2C0->SR = 0xff;
	LONGS_EQUAL(I2C_EVENT_MASK, i2c_get_event(PERI_I2C0));
	I2C0->SR = 0x40;
	LONGS_EQUAL(I2C_EVENT_TX, i2c_get_event(PERI_I2C0));
	I2C0->SR = 0x20;
	LONGS_EQUAL(I2C_EVENT_STOP, i2c_get_event(PERI_I2C0));
	I2C0->SR = 0x10;
	LONGS_EQUAL(I2C_EVENT_SLAVE, i2c_get_event(PERI_I2C0));
	I2C0->SR = 0x08;
	LONGS_EQUAL(I2C_EVENT_COLLISION, i2c_get_event(PERI_I2C0));
	I2C0->SR = 0x04;
	LONGS_EQUAL(I2C_EVENT_BUSY, i2c_get_event(PERI_I2C0));
	I2C0->SR = 0x01;
	LONGS_EQUAL(I2C_EVENT_RX, i2c_get_event(PERI_I2C0));
}
