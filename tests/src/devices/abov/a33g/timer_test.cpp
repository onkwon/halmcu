...
TEST(Timer, set_cc_ShouldSetCompareRegister) {
	timer_set_cc(PERIPH_TIMER0, 0, 1234);
	LONGS_EQUAL(1234, T0->GRA);
	timer_set_cc(PERIPH_TIMER1, 1, 5678);
...
TEST(Timer, get_cc_ShouldReturnCaptureRegister) {
	T0->GRA = 0xa5a5;
	T0->GRB = 0x5a5a;
	LONGS_EQUAL(0xa5a5, timer_get_cc(PERIPH_TIMER0, 0));
	LONGS_EQUAL(0x5a5a, timer_get_cc(PERIPH_TIMER0, 1));
}

TEST(Timer, get_cc_ShouldReturnZero_WhenUnsupportedChannelGiven) {
	LONGS_EQUAL(0, timer_get_cc(PERIPH_TIMER2, 0));
}
...
TEST(Timer, set_polarity_ShouldSetTstrt) {
	...
	timer_set_polarity(PERIPH_TIMER0, 0);
	LONGS_EQUAL(0, T0->CON);
}

TEST(Timer, set_edge_ShouldSetCapm) {
	timer_set_edge(PERIPH_TIMER0, TIMER_FALLING_EDGE);
