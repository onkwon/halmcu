...
TEST(Timer, set_prescaler_ShouldSetPrs) {
	timer_set_prescaler(PERI_TIMER0, 123);
	LONGS_EQUAL(123, T0->PRS);
	timer_set_prescaler(PERI_TIMER3, 2);
	LONGS_EQUAL(2, T3->PRS);
	timer_set_prescaler(PERI_TIMER9, 16);
	LONGS_EQUAL(16, T9->PRS);
}

TEST(Timer, set_clock_divider_ShouldSetTimerClockSource) {
	timer_set_clock_divider(PERI_TIMER0, 4);
	LONGS_EQUAL(0x40, T0->CON);
	timer_set_clock_divider(PERI_TIMER0, 6);
	LONGS_EQUAL(0x60, T0->CON);
	timer_set_clock_divider(PERI_TIMER0, 1);
	LONGS_EQUAL(0x10, T0->CON);
	timer_set_clock_divider(PERI_TIMER0, 0);
	LONGS_EQUAL(0, T0->CON);
}

TEST(Timer, get_counter_ShouldReturnCntValue) {
	T0->CNT = 123;
	LONGS_EQUAL(123, timer_get_counter(PERI_TIMER0));
	T4->CNT = 1;
	LONGS_EQUAL(1, timer_get_counter(PERI_TIMER4));
	T8->CNT = 2;
	LONGS_EQUAL(2, timer_get_counter(PERI_TIMER8));
}

TEST(Timer, set_reload_ShouldSetGra) {
	timer_set_reload(PERI_TIMER0, 123);
	LONGS_EQUAL(123, T0->GRA);
	timer_set_reload(PERI_TIMER2, 3);
	LONGS_EQUAL(3, T2->GRA);
	timer_set_reload(PERI_TIMER5, 4);
	LONGS_EQUAL(4, T5->GRA);
}

TEST(Timer, get_reload_ShouldReturnGra) {
	T0->GRA = 123;
	LONGS_EQUAL(123, timer_get_reload(PERI_TIMER0));
	T6->GRA = 5;
	LONGS_EQUAL(5, timer_get_reload(PERI_TIMER6));
	T7->GRA = 6;
	LONGS_EQUAL(6, timer_get_reload(PERI_TIMER7));
}

TEST(Timer, set_mode_ShouldSetConMode) {
	timer_set_mode(PERI_TIMER0, TIMER_MODE_PWM);
	LONGS_EQUAL(1, T0->CON);
	timer_set_mode(PERI_TIMER0, TIMER_MODE_CAPTURE);
	LONGS_EQUAL(3, T0->CON);
	timer_set_mode(PERI_TIMER0, TIMER_MODE_ONESHOT);
	LONGS_EQUAL(2, T0->CON);
	timer_set_mode(PERI_TIMER0, TIMER_MODE_NORMAL);
	LONGS_EQUAL(0, T0->CON);
}

TEST(Timer, enable_irq_ShouldSetIrqFlags) {
	timer_enable_irq(PERI_TIMER0, TIMER_EVENT_OVERFLOW);
	LONGS_EQUAL(0x400, T0->CON);
	timer_enable_irq(PERI_TIMER0, TIMER_EVENT_CC_0);
	LONGS_EQUAL(0x500, T0->CON);
	timer_enable_irq(PERI_TIMER0, TIMER_EVENT_CC_1);
	LONGS_EQUAL(0x700, T0->CON);
}

TEST(Timer, enable_irq_ShouldDoNothing_WhenUnsupportedGiven) {
	timer_enable_irq(PERI_TIMER0, TIMER_EVENT_CC_2);
	LONGS_EQUAL(0, T0->CON);
}

TEST(Timer, disable_irq_ShouldClearIrqFlags) {
	T1->CON = 0x700;
	timer_disable_irq(PERI_TIMER1, TIMER_EVENT_OVERFLOW);
	LONGS_EQUAL(0x300, T1->CON);
	timer_disable_irq(PERI_TIMER1, TIMER_EVENT_CC_0);
	LONGS_EQUAL(0x200, T1->CON);
	timer_disable_irq(PERI_TIMER1, TIMER_EVENT_CC_1);
	LONGS_EQUAL(0, T1->CON);
}

TEST(Timer, clear_event_ShouldClearIrqFlag) {
	timer_clear_event(PERI_TIMER0, TIMER_EVENT_OVERFLOW);
	LONGS_EQUAL(0x4000, T0->CON);
	timer_clear_event(PERI_TIMER0, TIMER_EVENT_CC_0);
	LONGS_EQUAL(0x5000, T0->CON);
	timer_clear_event(PERI_TIMER0, TIMER_EVENT_CC_1);
...
TEST(Timer, get_event_ShouldReturnCurrentFlags) {
	...
	LONGS_EQUAL(TIMER_EVENT_CC_0 | TIMER_EVENT_CC_1,
			timer_get_event(PERI_TIMER0));
	T0->CON = 0x4000;
	LONGS_EQUAL(TIMER_EVENT_OVERFLOW, timer_get_event(PERI_TIMER0));
}

TEST(Timer, set_polarity_ShouldSetTstrt) {
	timer_set_polarity(PERI_TIMER0, 1);
	LONGS_EQUAL(0x80, T0->CON);
	timer_set_polarity(PERI_TIMER0, 0);
	LONGS_EQUAL(0, T0->CON);
}

TEST(Timer, set_edge_ShouldSetCapm) {
	timer_set_edge(PERI_TIMER0, TIMER_FALLING_EDGE);
	LONGS_EQUAL(8, T0->CON);
	timer_set_edge(PERI_TIMER0, TIMER_RISING_EDGE);
	LONGS_EQUAL(0, T0->CON);
}
...
TEST(Timer, get_frequency_ShouldReturnTclk_WhenTCSIs4) {
	uint32_t tclk = 12345;
	T0->CON = 4U << 4;
	LONGS_EQUAL(tclk, timer_get_frequency(PERI_TIMER0, tclk));
}
TEST(Timer, get_frequency_ShouldReturnTclkDividedBy16_WhenTCSIs2) {
	uint32_t tclk = 6000;
	T0->CON = 2U << 4;
	LONGS_EQUAL(tclk/16, timer_get_frequency(PERI_TIMER0, tclk));
}
TEST(Timer, get_frequency_ShouldReturnZero_WhenTCSIs6) {
	uint32_t tclk = 6000;
	T0->CON = 6U << 4;
	LONGS_EQUAL(0, timer_get_frequency(PERI_TIMER0, tclk));
}
