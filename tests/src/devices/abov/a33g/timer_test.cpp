...
TEST(Timer, set_prescaler_ShouldSetPrs) {
	timer_ll_set_prescaler(PERIPH_TIMER0, 123);
	LONGS_EQUAL(123, T0->PRS);
	timer_ll_set_prescaler(PERIPH_TIMER3, 2);
...
TEST(Timer, set_clock_divider_ShouldSetTimerClockSource) {
	...
	timer_ll_set_clock_divider(PERIPH_TIMER0, 6);
	LONGS_EQUAL(0x60, T0->CON);
	timer_ll_set_clock_divider(PERIPH_TIMER0, 1);
	LONGS_EQUAL(0x10, T0->CON);
	timer_ll_set_clock_divider(PERIPH_TIMER0, 0);
	LONGS_EQUAL(0, T0->CON);
}

TEST(Timer, get_counter_ShouldReturnCntValue) {
	T0->CNT = 123;
	LONGS_EQUAL(123, timer_ll_get_counter(PERIPH_TIMER0));
	T4->CNT = 1;
	LONGS_EQUAL(1, timer_ll_get_counter(PERIPH_TIMER4));
	T8->CNT = 2;
	LONGS_EQUAL(2, timer_ll_get_counter(PERIPH_TIMER8));
}

TEST(Timer, set_reload_ShouldSetGra) {
	timer_ll_set_reload(PERIPH_TIMER0, 123);
	LONGS_EQUAL(123, T0->GRA);
	timer_ll_set_reload(PERIPH_TIMER2, 3);
	LONGS_EQUAL(3, T2->GRA);
	timer_ll_set_reload(PERIPH_TIMER5, 4);
	LONGS_EQUAL(4, T5->GRA);
}

TEST(Timer, get_reload_ShouldReturnGra) {
	T0->GRA = 123;
	LONGS_EQUAL(123, timer_ll_get_reload(PERIPH_TIMER0));
	T6->GRA = 5;
	LONGS_EQUAL(5, timer_ll_get_reload(PERIPH_TIMER6));
	T7->GRA = 6;
	LONGS_EQUAL(6, timer_ll_get_reload(PERIPH_TIMER7));
}

TEST(Timer, set_mode_ShouldSetConMode) {
	timer_ll_set_mode(PERIPH_TIMER0, TIMER_MODE_PWM);
	LONGS_EQUAL(1, T0->CON);
	timer_ll_set_mode(PERIPH_TIMER0, TIMER_MODE_CAPTURE);
	LONGS_EQUAL(3, T0->CON);
	timer_ll_set_mode(PERIPH_TIMER0, TIMER_MODE_ONESHOT);
	LONGS_EQUAL(2, T0->CON);
	timer_ll_set_mode(PERIPH_TIMER0, TIMER_MODE_NORMAL);
	LONGS_EQUAL(0, T0->CON);
}

TEST(Timer, enable_irq_ShouldSetIrqFlags) {
	timer_ll_enable_irq(PERIPH_TIMER0, TIMER_EVENT_OVERFLOW);
	LONGS_EQUAL(0x400, T0->CON);
	timer_ll_enable_irq(PERIPH_TIMER0, TIMER_EVENT_CC_0);
	LONGS_EQUAL(0x500, T0->CON);
	timer_ll_enable_irq(PERIPH_TIMER0, TIMER_EVENT_CC_1);
	LONGS_EQUAL(0x700, T0->CON);
}

TEST(Timer, enable_irq_ShouldDoNothing_WhenUnsupportedGiven) {
	timer_ll_enable_irq(PERIPH_TIMER0, TIMER_EVENT_CC_2);
	LONGS_EQUAL(0, T0->CON);
}

TEST(Timer, disable_irq_ShouldClearIrqFlags) {
	T1->CON = 0x700;
	timer_ll_disable_irq(PERIPH_TIMER1, TIMER_EVENT_OVERFLOW);
	LONGS_EQUAL(0x300, T1->CON);
	timer_ll_disable_irq(PERIPH_TIMER1, TIMER_EVENT_CC_0);
	LONGS_EQUAL(0x200, T1->CON);
	timer_ll_disable_irq(PERIPH_TIMER1, TIMER_EVENT_CC_1);
	LONGS_EQUAL(0, T1->CON);
}

TEST(Timer, clear_event_ShouldClearIrqFlag) {
	timer_ll_clear_event(PERIPH_TIMER0, TIMER_EVENT_OVERFLOW);
	LONGS_EQUAL(0x4000, T0->CON);
	timer_ll_clear_event(PERIPH_TIMER0, TIMER_EVENT_CC_0);
	LONGS_EQUAL(0x5000, T0->CON);
	timer_ll_clear_event(PERIPH_TIMER0, TIMER_EVENT_CC_1);
	LONGS_EQUAL(0x7000, T0->CON);
}

TEST(Timer, start_ShouldSetTEN) {
	timer_ll_start(PERIPH_TIMER0);
	LONGS_EQUAL(3, T0->CMD); /* TEN + TCLR */
}

TEST(Timer, stop_ShouldClearTEN) {
	T0->CMD = 1;
	timer_ll_stop(PERIPH_TIMER0);
...
TEST(Timer, get_event_ShouldReturnCurrentFlags) {
	...
	LONGS_EQUAL(TIMER_EVENT_CC_0 | TIMER_EVENT_CC_1,
			timer_ll_get_event(PERIPH_TIMER0));
	T0->CON = 0x4000;
	LONGS_EQUAL(TIMER_EVENT_OVERFLOW, timer_ll_get_event(PERIPH_TIMER0));
}
...
TEST(Timer, get_frequency_ShouldReturnTclk_WhenTCSIs4) {
	uint32_t tclk = 12345;
	T0->CON = 4U << 4;
	LONGS_EQUAL(tclk, timer_ll_get_frequency(PERIPH_TIMER0, tclk));
}
TEST(Timer, get_frequency_ShouldReturnTclkDividedBy16_WhenTCSIs2) {
	uint32_t tclk = 6000;
	T0->CON = 2U << 4;
	LONGS_EQUAL(tclk/16, timer_ll_get_frequency(PERIPH_TIMER0, tclk));
}
