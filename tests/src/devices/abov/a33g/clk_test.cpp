...
TEST(Clock, enable_peripheral_ShouldActivateClock) {
	...
	clk_enable_peripheral(PERIPH_UART2);
	LONGS_EQUAL(0x10400118, PMU->PCCR);
	clk_enable_peripheral(PERIPH_I2C0);
	LONGS_EQUAL(0x10440118, PMU->PCCR);
	clk_enable_peripheral(PERIPH_SPI1);
	LONGS_EQUAL(0x10460118, PMU->PCCR);
	clk_enable_peripheral(PERIPH_GPIOA);
	LONGS_EQUAL(0x10460118, PMU->PCCR);
}

TEST(Clock, disable_peripheral_ShouldDeactivateClock) {
	PMU->PCCR = 0xffffffff;
	clk_disable_peripheral(PERIPH_ADC);
	...
	clk_disable_peripheral(PERIPH_FRT);
...
TEST(Clock, get_peripheral_clock_source_ShouldReturnClockSource) {
	PMU->PCSR = 0x390;
	LONGS_EQUAL(CLK_LSI, clk_get_peripheral_clock_source(PERIPH_TIMER9));
	LONGS_EQUAL(CLK_LSE, clk_get_peripheral_clock_source(PERIPH_TIMER5));
	LONGS_EQUAL(CLK_HSI, clk_get_peripheral_clock_source(PERIPH_TIMER1));
	PMU->PCSR = 0xE0;
	LONGS_EQUAL(CLK_PLL, clk_get_peripheral_clock_source(PERIPH_TIMER9));
	LONGS_EQUAL(CLK_LSI, clk_get_peripheral_clock_source(PERIPH_TIMER5));
	LONGS_EQUAL(CLK_LSE, clk_get_peripheral_clock_source(PERIPH_TIMER1));
}
