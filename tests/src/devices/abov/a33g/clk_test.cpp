...
TEST(Clock, enable_peripheral_ShouldActivateClock) {
	clk_ll_enable_peripheral(PERIPH_ADC);
	LONGS_EQUAL(0x10000118, PMU->PCCR);
	clk_ll_enable_peripheral(PERIPH_UART2);
	LONGS_EQUAL(0x10400118, PMU->PCCR);
	clk_ll_enable_peripheral(PERIPH_I2C0);
	LONGS_EQUAL(0x10440118, PMU->PCCR);
	clk_ll_enable_peripheral(PERIPH_SPI1);
	LONGS_EQUAL(0x10460118, PMU->PCCR);
	clk_ll_enable_peripheral(PERIPH_GPIOA);
	LONGS_EQUAL(0x10460118, PMU->PCCR);
}

TEST(Clock, disable_peripheral_ShouldDeactivateClock) {
	PMU->PCCR = 0xffffffff;
	clk_ll_disable_peripheral(PERIPH_ADC);
	...
	clk_ll_disable_peripheral(PERIPH_FRT);
...
TEST(Clock, set_source_ShouldSelectSourceClock) {
	...
	clk_ll_set_source(CLK_LSI);
	LONGS_EQUAL(0, PMU->BCCR);
	clk_ll_set_source(CLK_LSE);
	LONGS_EQUAL(1, PMU->BCCR);
}

TEST(Clock, get_source_ShouldReturnSourceClock) {
	LONGS_EQUAL(CLK_LSI, clk_ll_get_source());
	clk_ll_set_source(CLK_PLL);
	LONGS_EQUAL(CLK_PLL, clk_ll_get_source());
	clk_ll_set_source(CLK_LSE);
	LONGS_EQUAL(CLK_LSE, clk_ll_get_source());
}
...
TEST(Clock, set_pll_freq_ShouldConfigurePLL_WhenOSC16MHzGiven) {
	LONGS_EQUAL(1, clk_ll_set_pll_frequency(CLK_PLL, CLK_HSI, 8*MHZ));
	clk_ll_set_source(CLK_PLL);
	LONGS_EQUAL(0x300700, PMU->PLLCON & 0xFFFFFFF);
	LONGS_EQUAL(0x2, PMU->BCCR);
	LONGS_EQUAL(0xA0, PMU->CCR);

	LONGS_EQUAL(1, clk_ll_set_pll_frequency(CLK_PLL, CLK_HSI, 75*MHZ));
...
TEST(Clock, set_pll_freq_ShouldConfigurePLL_WhenXtalGiven) {
	LONGS_EQUAL(1, clk_ll_set_pll_frequency(CLK_PLL, CLK_HSE, 8*MHZ));
	clk_ll_set_source(CLK_PLL);
	LONGS_EQUAL(0x300300, PMU->PLLCON & 0xFFFFFFF);
	LONGS_EQUAL(0x12, PMU->BCCR);
	LONGS_EQUAL(0x82, PMU->CCR);

	LONGS_EQUAL(1, clk_ll_set_pll_frequency(CLK_PLL, CLK_HSE, 75*MHZ));
...
TEST(Clock, get_hclk_freq_ShouldReturnHclk) {
	LONGS_EQUAL(1*MHZ, clk_ll_get_hclk_frequency());
	clk_ll_set_pll_frequency(CLK_PLL, CLK_HSI, 8*MHZ);
	clk_ll_set_source(CLK_PLL);
	LONGS_EQUAL(8*MHZ, clk_ll_get_hclk_frequency());
	clk_ll_set_pll_frequency(CLK_PLL, CLK_HSI, 75*MHZ);
	clk_ll_set_source(CLK_PLL);
	LONGS_EQUAL(75*MHZ, clk_ll_get_hclk_frequency());
	clk_ll_set_pll_frequency(CLK_PLL, CLK_HSE, 75*MHZ);
	clk_ll_set_source(CLK_PLL);
	LONGS_EQUAL(75*MHZ, clk_ll_get_hclk_frequency());
}

TEST(Clock, get_pclk_freq_ShouldReturnHclk) {
	LONGS_EQUAL(1*MHZ, clk_ll_get_pclk_frequency());
	clk_ll_set_pll_frequency(CLK_PLL, CLK_HSI, 8*MHZ);
	clk_ll_set_source(CLK_PLL);
	LONGS_EQUAL(8*MHZ, clk_ll_get_pclk_frequency());
	clk_ll_set_pll_frequency(CLK_PLL, CLK_HSE, 32*MHZ);
	clk_ll_set_source(CLK_PLL);
	LONGS_EQUAL(32*MHZ, clk_ll_get_pclk_frequency());
}

TEST(Clock, get_freq_ShouldReturnClk) {
	LONGS_EQUAL(1000000, clk_ll_get_frequency(CLK_PLL));
	LONGS_EQUAL(8000000, clk_ll_get_frequency(CLK_HSE));
	LONGS_EQUAL(0, clk_ll_get_frequency(CLK_LSE));
	LONGS_EQUAL(1000000, clk_ll_get_frequency(CLK_LSI));
	LONGS_EQUAL(16000000, clk_ll_get_frequency(CLK_HSI));
}

TEST(Clock, is_pll_locked_ShouldReturnPllStatus) {
	LONGS_EQUAL(0, clk_ll_is_pll_locked());
	PMU->PLLCON |= 1U << 12;
	LONGS_EQUAL(1, clk_ll_is_pll_locked());
}

TEST(Clock, get_peripheral_clock_source_ShouldReturnClockSource) {
	PMU->PCSR = 0x390;
	LONGS_EQUAL(CLK_LSI, clk_ll_get_peripheral_clock_source(PERIPH_TIMER9));
	LONGS_EQUAL(CLK_LSE, clk_ll_get_peripheral_clock_source(PERIPH_TIMER5));
	LONGS_EQUAL(CLK_HSI, clk_ll_get_peripheral_clock_source(PERIPH_TIMER1));
	PMU->PCSR = 0xE0;
	LONGS_EQUAL(CLK_PLL, clk_ll_get_peripheral_clock_source(PERIPH_TIMER9));
	LONGS_EQUAL(CLK_LSI, clk_ll_get_peripheral_clock_source(PERIPH_TIMER5));
	LONGS_EQUAL(CLK_LSE, clk_ll_get_peripheral_clock_source(PERIPH_TIMER1));
}
