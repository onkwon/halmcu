...
TEST(SPI, set_chip_select_mode_ShouldSetSSMODE) {
	spi_set_chip_select_mode(PERIPH_SPI0, true);
	LONGS_EQUAL(0x1000, SPI0->CR & 0x1000);
	spi_set_chip_select_mode(PERIPH_SPI0, false);
	LONGS_EQUAL(0, SPI0->CR & 0x1000);
}

TEST(SPI, set_chip_select_level_ShouldSetSSOUT) {
	spi_set_chip_select_level(PERIPH_SPI0, 1);
	LONGS_EQUAL(0x800, SPI0->CR & 0x800);
	spi_set_chip_select_level(PERIPH_SPI0, 0);
	LONGS_EQUAL(0, SPI0->CR & 0x800);
}

TEST(SPI, set_chip_select_polarity_ShouldSetSSPOL) {
	spi_set_chip_select_polarity(PERIPH_SPI0, 1);
	LONGS_EQUAL(0x80, SPI0->CR & 0x80);
	spi_set_chip_select_polarity(PERIPH_SPI0, 0);
...
TEST(SPI, set_loopback_ShouldSetLBE) {
	...
	spi_set_loopback(PERIPH_SPI0, false);
...
TEST(SPI, set_mode_ShouldSetMS) {
	...
	spi_set_mode(PERIPH_SPI0, SPI_MODE_SLAVE);
	LONGS_EQUAL(0, SPI0->CR & 0x20);
}

TEST(SPI, set_clock_phase_ShouldSetCPHA) {
	spi_set_clock_phase(PERIPH_SPI0, 1);
...
TEST(SPI, set_clock_polarity_ShouldCPOL) {
	spi_set_clock_polarity(PERIPH_SPI0, 1);
...
TEST(SPI, set_data_width_ShouldSetBITSEL) {
	spi_set_data_width(PERIPH_SPI0, 17);
	LONGS_EQUAL(3, SPI0->CR & 0x3);
	spi_set_data_width(PERIPH_SPI0, 16);
	LONGS_EQUAL(2, SPI0->CR & 0x3);
	spi_set_data_width(PERIPH_SPI0, 9);
...
TEST(SPI, set_bitorder_ShouldSetMSBF) {
	...
	spi_set_bitorder(PERIPH_SPI0, true);
	LONGS_EQUAL(0, SPI0->CR & 0x10);
}

TEST(SPI, enable_irq_ShouldEnableInterrupts) {
	spi_enable_irq(PERIPH_SPI0, SPI_IRQ_RX);
	LONGS_EQUAL(0x2000, SPI0->CR & 0xE000);
	spi_enable_irq(PERIPH_SPI0, SPI_IRQ_TX);
	LONGS_EQUAL(0x6000, SPI0->CR & 0xE000);
	spi_enable_irq(PERIPH_SPI0, SPI_IRQ_EDGE_CHAGNE);
	LONGS_EQUAL(0xE000, SPI0->CR & 0xE000);
}

TEST(SPI, disable_irq_ShouldDisableInterrupts) {
	spi_enable_irq(PERIPH_SPI0, (spi_irq_t)(SPI_IRQ_RX | SPI_IRQ_TX | SPI_IRQ_EDGE_CHAGNE));
	spi_disable_irq(PERIPH_SPI0, SPI_IRQ_EDGE_CHAGNE);
	LONGS_EQUAL(0x6000, SPI0->CR & 0xE000);
	spi_disable_irq(PERIPH_SPI0, SPI_IRQ_TX);
	LONGS_EQUAL(0x2000, SPI0->CR & 0xE000);
	spi_disable_irq(PERIPH_SPI0, SPI_IRQ_RX);
	LONGS_EQUAL(0, SPI0->CR & 0xE000);
}

TEST(SPI, set_frequency_ShouldSetBR) {
	spi_set_frequency(PERIPH_SPI0, 1000000, 16000000);
	LONGS_EQUAL(16-1, SPI0->BR);
}

TEST(SPI, start_ShouldSetEN) {
	spi_enable_clock(PERIPH_SPI0);
	LONGS_EQUAL(1, SPI0->EN);
}

TEST(SPI, stop_ShouldClearEN) {
	spi_enable_clock(PERIPH_SPI0);
	spi_disable_clock(PERIPH_SPI0);
	LONGS_EQUAL(0, SPI0->EN);
}

TEST(SPI, get_event_ShouldReturnSR) {
	SPI0->SR = 0x7f;
	LONGS_EQUAL(0x7f, spi_get_event(PERIPH_SPI0));
}

TEST(SPI, clear_event_ShouldClearSR) {
	SPI0->SR = 0x7f;
	spi_clear_event(PERIPH_SPI0, SPI_EVENT_OVERRUN);
	LONGS_EQUAL(0x6f, SPI0->SR);
	spi_clear_event(PERIPH_SPI0, SPI_EVENT_UNDERRUN);
	LONGS_EQUAL(0x67, SPI0->SR);
	spi_clear_event(PERIPH_SPI0, SPI_EVENT_CHIP_DESELECTED);
