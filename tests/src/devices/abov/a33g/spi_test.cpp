...
TEST(SPI, set_loopback_ShouldSetLBE) {
	...
	spi_set_loopback(PERI_SPI0, false);
...
TEST(SPI, set_mode_ShouldSetMS) {
	...
	spi_set_mode(PERI_SPI0, SPI_MODE_SLAVE);
...
TEST(SPI, set_clock_polarity_ShouldCPOL) {
	spi_set_clock_polarity(PERI_SPI0, 1);
...
TEST(SPI, set_data_width_ShouldSetBITSEL) {
	spi_set_data_width(PERI_SPI0, 17);
	LONGS_EQUAL(3, SPI0->CR & 0x3);
	spi_set_data_width(PERI_SPI0, 16);
	LONGS_EQUAL(2, SPI0->CR & 0x3);
	spi_set_data_width(PERI_SPI0, 9);
...
TEST(SPI, set_bitorder_ShouldSetMSBF) {
	...
	spi_set_bitorder(PERI_SPI0, true);
	LONGS_EQUAL(0, SPI0->CR & 0x10);
}

TEST(SPI, enable_irq_ShouldEnableInterrupts) {
	spi_enable_irq(PERI_SPI0, SPI_IRQ_RX);
	LONGS_EQUAL(0x2000, SPI0->CR & 0xE000);
	spi_enable_irq(PERI_SPI0, SPI_IRQ_TX);
	LONGS_EQUAL(0x6000, SPI0->CR & 0xE000);
	spi_enable_irq(PERI_SPI0, SPI_IRQ_EDGE_CHAGNE);
	LONGS_EQUAL(0xE000, SPI0->CR & 0xE000);
}

TEST(SPI, disable_irq_ShouldDisableInterrupts) {
	spi_enable_irq(PERI_SPI0, (spi_irq_t)(SPI_IRQ_RX | SPI_IRQ_TX | SPI_IRQ_EDGE_CHAGNE));
	spi_disable_irq(PERI_SPI0, SPI_IRQ_EDGE_CHAGNE);
	LONGS_EQUAL(0x6000, SPI0->CR & 0xE000);
	spi_disable_irq(PERI_SPI0, SPI_IRQ_TX);
	LONGS_EQUAL(0x2000, SPI0->CR & 0xE000);
	spi_disable_irq(PERI_SPI0, SPI_IRQ_RX);
