...
TEST(gpio, write_ShouldSetSpecificPin) {
	gpio_write_pin(PERIPH_GPIOA, 0, 0);
	LONGS_EQUAL(0x10000, PA->SRR);
	gpio_write_pin(PERIPH_GPIOA, 0, 1);
	LONGS_EQUAL(0x1, PA->SRR);
	gpio_write_pin(PERIPH_GPIOB, 15, 0);
	LONGS_EQUAL(0x80000000, PB->SRR);
	gpio_write_pin(PERIPH_GPIOB, 15, 1);
	LONGS_EQUAL(0x8000, PB->SRR);
}

TEST(gpio, write_ShouldCauseAssert_WhenUnsupportedGpioGiven) {
	mock().expectOneCall("halmcu_assertion_failed");
	gpio_write_pin(PERIPH_GPIOA, 16, 1);
}

TEST(gpio, read_ShouldReturnCurrentPinState) {
	LONGS_EQUAL(0, gpio_read_pin(PERIPH_GPIOA, 0));
	PA->IDR = 1;
	LONGS_EQUAL(1, gpio_read_pin(PERIPH_GPIOA, 0));
	PB->IDR = 0xA5;
	LONGS_EQUAL(1, gpio_read_pin(PERIPH_GPIOB, 0));
	LONGS_EQUAL(0, gpio_read_pin(PERIPH_GPIOB, 1));
	LONGS_EQUAL(1, gpio_read_pin(PERIPH_GPIOB, 2));
	LONGS_EQUAL(0, gpio_read_pin(PERIPH_GPIOB, 3));
	LONGS_EQUAL(0, gpio_read_pin(PERIPH_GPIOB, 4));
	LONGS_EQUAL(1, gpio_read_pin(PERIPH_GPIOB, 5));
	LONGS_EQUAL(0, gpio_read_pin(PERIPH_GPIOB, 6));
	LONGS_EQUAL(1, gpio_read_pin(PERIPH_GPIOB, 7));
}

TEST(gpio, read_ShouldCauseAssert_WhenUnsupportedGpioGiven) {
	mock().expectOneCall("halmcu_assertion_failed");
	gpio_read_pin(PERIPH_GPIOA, 16);
}
