...
TEST(GPIO, read_ShouldReturnIDRBit) {
	...
	LONGS_EQUAL(0, gpio_read_pin(PERIPH_GPIOA, 1));
	LONGS_EQUAL(0, gpio_read_pin(PERIPH_GPIOA, 4));
	LONGS_EQUAL(1, gpio_read_pin(PERIPH_GPIOA, 7));
	GPIOB->IDR = 0x5A;
	LONGS_EQUAL(0, gpio_read_pin(PERIPH_GPIOB, 0));
	LONGS_EQUAL(1, gpio_read_pin(PERIPH_GPIOB, 1));
	LONGS_EQUAL(1, gpio_read_pin(PERIPH_GPIOB, 4));
	LONGS_EQUAL(0, gpio_read_pin(PERIPH_GPIOB, 7));
}

TEST(GPIO, write_ShouldCauseAssert_WhenInvalidPortGiven) {
	mock().expectOneCall("halmcu_assertion_failed");
	gpio_write_pin(PERIPH_USART1, 0, 0);
}
TEST(GPIO, write_ShouldCauseAssert_WhenInvalidPinGiven) {
	mock().expectOneCall("halmcu_assertion_failed");
	gpio_write_pin(PERIPH_GPIOA, 16, 0);
}
TEST(GPIO, write_ShouldSetBSRRBit) {
	gpio_write_pin(PERIPH_GPIOE, 0, 1);
	LONGS_EQUAL(1, GPIOE->BSRR);
	gpio_write_pin(PERIPH_GPIOE, 1, 0);
	LONGS_EQUAL(0x20000, GPIOE->BSRR);
	gpio_write_pin(PERIPH_GPIOE, 15, 0);
	LONGS_EQUAL(0x80000000, GPIOE->BSRR);
	gpio_write_pin(PERIPH_GPIOE, 14, 1);
