...
TEST(GPIO, read_ShouldReturnIDRBit) {
	...
	LONGS_EQUAL(0, gpio_ll_read(PERIPH_GPIOA, 1));
	LONGS_EQUAL(0, gpio_ll_read(PERIPH_GPIOA, 4));
	LONGS_EQUAL(1, gpio_ll_read(PERIPH_GPIOA, 7));
	GPIOB->IDR = 0x5A;
	LONGS_EQUAL(0, gpio_ll_read(PERIPH_GPIOB, 0));
	LONGS_EQUAL(1, gpio_ll_read(PERIPH_GPIOB, 1));
	LONGS_EQUAL(1, gpio_ll_read(PERIPH_GPIOB, 4));
	LONGS_EQUAL(0, gpio_ll_read(PERIPH_GPIOB, 7));
}

TEST(GPIO, write_ShouldCauseAssert_WhenInvalidPortGiven) {
	mock().expectOneCall("abov_assertion_failed");
	gpio_ll_write(PERIPH_USART1, 0, 0);
}
TEST(GPIO, write_ShouldCauseAssert_WhenInvalidPinGiven) {
	mock().expectOneCall("abov_assertion_failed");
	gpio_ll_write(PERIPH_GPIOA, 16, 0);
}
TEST(GPIO, write_ShouldSetBSRRBit) {
	gpio_ll_write(PERIPH_GPIOE, 0, 1);
	LONGS_EQUAL(1, GPIOE->BSRR);
	gpio_ll_write(PERIPH_GPIOE, 1, 0);
	LONGS_EQUAL(0x20000, GPIOE->BSRR);
	gpio_ll_write(PERIPH_GPIOE, 15, 0);
	LONGS_EQUAL(0x80000000, GPIOE->BSRR);
	gpio_ll_write(PERIPH_GPIOE, 14, 1);
	LONGS_EQUAL(0x4000, GPIOE->BSRR);
}

TEST(GPIO, enable_irq_ShouldCauseAssert_WhenInvalidPortGiven) {
	mock().expectOneCall("abov_assertion_failed");
	gpio_ll_enable_irq(PERIPH_USART1, 0, GPIO_IRQ_EDGE_ANY);
}
TEST(GPIO, enable_irq_ShouldCauseAssert_WhenInvalidPinGiven) {
	mock().expectOneCall("abov_assertion_failed");
	gpio_ll_enable_irq(PERIPH_GPIOA, 20, GPIO_IRQ_EDGE_ANY);
}
...
TEST(GPIO, enable_irq_ShouldSetRisingEdge_WhenRisingOptionGiven) {
	...
	gpio_ll_enable_irq(PERIPH_GPIOA, 2, GPIO_IRQ_EDGE_RISING);
}
TEST(GPIO, enable_irq_ShouldSetFallingEdge_WhenFallingOptionGiven) {
	...
	gpio_ll_enable_irq(PERIPH_GPIOA, 3, GPIO_IRQ_EDGE_FALLING);
}
...
TEST(GPIO, set_mode_ShouldSetCRL_WhenPin0To7Given) {
	...
	gpio_ll_set_mode(PERIPH_GPIOA, 2, GPIO_MODE_OPENDRAIN);
	LONGS_EQUAL(0x620, GPIOA->CRL);
	gpio_ll_set_mode(PERIPH_GPIOA, 3, GPIO_MODE_INPUT_PULLUP);
	LONGS_EQUAL(0x8620, GPIOA->CRL);
}
TEST(GPIO, set_mode_ShouldSetCRH_WhenPin8To15Given) {
	gpio_ll_set_mode(PERIPH_GPIOA, 15, GPIO_MODE_INPUT);
	LONGS_EQUAL(0x40000000, GPIOA->CRH);
	gpio_ll_set_mode(PERIPH_GPIOA, 13, GPIO_MODE_PUSHPULL);
	LONGS_EQUAL(0x40200000, GPIOA->CRH);
}

TEST(GPIO, set_altfunc_ShouldCauseAssert_WhenInvalidPortGiven) {
	mock().expectOneCall("abov_assertion_failed");
	gpio_ll_set_altfunc(PERIPH_USART1, 0, 0);
}
TEST(GPIO, set_altfunc_ShouldCauseAssert_WhenInvalidPinGiven) {
	mock().expectOneCall("abov_assertion_failed");
	gpio_ll_set_altfunc(PERIPH_GPIOA, 20, 0);
}
TEST(GPIO, set_altfunc_ShouldSetCRL_WhenPin0To7AndPushpullGiven) {
	gpio_ll_set_mode(PERIPH_GPIOG, 0, GPIO_MODE_PUSHPULL);
	gpio_ll_set_altfunc(PERIPH_GPIOG, 0, 0);
...
TEST(GPIO, set_altfunc_ShouldSetCRH_WhenPin8To15AndPushpullGiven) {
	...
	gpio_ll_set_altfunc(PERIPH_GPIOA, 15, 0);
...
TEST(GPIO, set_altfunc_ShouldSetCR_WhenInputPulldownModeGiven) {
	gpio_ll_set_mode(PERIPH_GPIOA, 0, GPIO_MODE_INPUT_PULLDOWN);
	gpio_ll_set_altfunc(PERIPH_GPIOA, 0, 0);
	LONGS_EQUAL(8, GPIOA->CRL);
}
...
TEST(GPIO, set_speed_ShouldCauseAssert_WhenInvalidPortGiven) {
	mock().expectOneCall("abov_assertion_failed");
	gpio_ll_set_speed(PERIPH_USART1, 0, GPIO_SPEED_LOW);
}
TEST(GPIO, set_speed_ShouldCauseAssert_WhenInvalidPinGiven) {
	mock().expectOneCall("abov_assertion_failed");
	gpio_ll_set_speed(PERIPH_GPIOA, 16, GPIO_SPEED_LOW);
}
TEST(GPIO, set_speed_ShouldSetSpeed) {
	gpio_ll_set_mode(PERIPH_GPIOA, 0, GPIO_MODE_PUSHPULL);
	gpio_ll_set_speed(PERIPH_GPIOA, 0, GPIO_SPEED_HIGH);
	LONGS_EQUAL(3, GPIOA->CRL);
	gpio_ll_set_mode(PERIPH_GPIOA, 1, GPIO_MODE_PUSHPULL);
	gpio_ll_set_speed(PERIPH_GPIOA, 1, GPIO_SPEED_MID);
	LONGS_EQUAL(0x13, GPIOA->CRL);
	gpio_ll_set_speed(PERIPH_GPIOA, 0, GPIO_SPEED_LOW);
...
TEST(GPIO, set_speed_ShouldSetSpeed_WhenOpendrainModeGiven) {
	gpio_ll_set_mode(PERIPH_GPIOA, 0, GPIO_MODE_OPENDRAIN);
	gpio_ll_set_speed(PERIPH_GPIOA, 0, GPIO_SPEED_MID);
