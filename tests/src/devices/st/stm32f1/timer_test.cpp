...
TEST(Timer, reset_ShouldSetAndClearRSTR_WhenTIM7Given) {
	...
	mock().expectOneCall("bitop_clear")
		.withParameter("pos", 5)
		.ignoreOtherParameters();
	timer_reset(PERIPH_TIM7);
}

TEST(Timer, set_prescaler_ShouldSetPSC) {
	timer_set_prescaler(PERIPH_TIM7, 0x1234);
	LONGS_EQUAL(0x1234, TIM7->PSC);
}
TEST(Timer, get_prescaler_ShouldReturnPSC) {
	TIM1->PSC = 0xcafe;
	LONGS_EQUAL(0xcafe, timer_get_prescaler(PERIPH_TIM1));
}

TEST(Timer, enable_irq_ShouldSetDIER) {
	timer_enable_irq(PERIPH_TIM5, TIMER_EVENT_CC_1);
	LONGS_EQUAL(2, TIM5->DIER);
	timer_enable_irq(PERIPH_TIM5, TIMER_EVENT_CC_2);
	LONGS_EQUAL(6, TIM5->DIER);
	timer_enable_irq(PERIPH_TIM5, TIMER_EVENT_UPDATE);
	LONGS_EQUAL(7, TIM5->DIER);
}

TEST(Timer, disable_irq_ShouldClearDIER) {
	TIM1->DIER = 0xFu;
	timer_disable_irq(PERIPH_TIM1, TIMER_EVENT_CC_1);
	LONGS_EQUAL(0xd, TIM1->DIER);
	timer_disable_irq(PERIPH_TIM1, TIMER_EVENT_CC_2);
	LONGS_EQUAL(9, TIM1->DIER);
	timer_disable_irq(PERIPH_TIM1, TIMER_EVENT_UPDATE);
	LONGS_EQUAL(8, TIM1->DIER);
}

TEST(Timer, set_counter_ShouldSetCNT) {
	timer_set_counter(PERIPH_TIM1, 0x1234);
	LONGS_EQUAL(0x1234, TIM1->CNT);
}
TEST(Timer, get_counter_ShouldReturnCNT) {
	TIM1->CNT = 0xfeed;
	LONGS_EQUAL(0xfeed, timer_get_counter(PERIPH_TIM1));
}

TEST(Timer, set_reload_ShouldSetARR) {
	timer_set_reload(PERIPH_TIM3, 0x1234);
	LONGS_EQUAL(0x1234, TIM3->ARR);
}
TEST(Timer, get_reload_ShouldReturnARR) {
	TIM4->ARR = 0xfeed;
	LONGS_EQUAL(0xfeed, timer_get_reload(PERIPH_TIM4));
}
...
TEST(Timer, stop_ShouldClearCEN) {
	mock().expectOneCall("bitop_clear")
		.withParameter("pos", 0)
		.ignoreOtherParameters();
	timer_stop(PERIPH_TIM2);
}

TEST(Timer, set_cc_ShouldSetCCR) {
	timer_set_cc(PERIPH_TIM1, TIMER_CC_1, 0xc0de);
	timer_set_cc(PERIPH_TIM1, TIMER_CC_2, 0xc1de);
	timer_set_cc(PERIPH_TIM1, TIMER_CC_3, 0xc2de);
	timer_set_cc(PERIPH_TIM1, TIMER_CC_4, 0xc3de);
...
TEST(Timer, get_cc_ShouldReturnCCR) {
	...
	LONGS_EQUAL(0xc1de, timer_get_cc(PERIPH_TIM1, TIMER_CC_1));
	LONGS_EQUAL(0xc2de, timer_get_cc(PERIPH_TIM1, TIMER_CC_2));
	LONGS_EQUAL(0xc3de, timer_get_cc(PERIPH_TIM1, TIMER_CC_3));
	LONGS_EQUAL(0xc4de, timer_get_cc(PERIPH_TIM1, TIMER_CC_4));
}
TEST(Timer, get_cc_ShouldReturnZero_WhenInvalidParamGiven) {
	LONGS_EQUAL(0, timer_get_cc(PERIPH_TIM1, TIMER_CC_0));
	LONGS_EQUAL(0, timer_get_cc(PERIPH_TIM1, TIMER_CC_1N));
}

TEST(Timer, clear_event_ShouldClearSR) {
	TIM1->SR = 0x1f;
	timer_clear_event(PERIPH_TIM1, TIMER_EVENT_CC_1);
	LONGS_EQUAL(0x1d, TIM1->SR);
	timer_clear_event(PERIPH_TIM1, TIMER_EVENT_CC_2);
	LONGS_EQUAL(0x19, TIM1->SR);
	timer_clear_event(PERIPH_TIM1, TIMER_EVENT_CC_3);
	LONGS_EQUAL(0x11, TIM1->SR);
	timer_clear_event(PERIPH_TIM1, TIMER_EVENT_CC_4);
	LONGS_EQUAL(0x1, TIM1->SR);
	timer_clear_event(PERIPH_TIM1, TIMER_EVENT_UPDATE);
...
TEST(Timer, get_event_ShouldReturnSR) {
	...
	LONGS_EQUAL(TIMER_EVENT_CC_1 | TIMER_EVENT_UPDATE,
			timer_get_event(PERIPH_TIM1));
	TIM1->SR = 0x4;
	LONGS_EQUAL(TIMER_EVENT_CC_2, timer_get_event(PERIPH_TIM1));
	TIM1->SR = 0x18;
...
TEST(Timer, get_frequency_ShouldReturnSourceClockFrequencyDividedByTwo) {
	RCC->CFGR = (1U << 13) | (1 << 10);
	LONGS_EQUAL(1000000, timer_get_frequency(PERIPH_TIM1, 2000000));
}
TEST(Timer, get_frequency_ShouldReturnSourceClockFrequency) {
	LONGS_EQUAL(1000000, timer_get_frequency(PERIPH_TIM1, 1000000));
}

TEST(Timer, set_clock_divider_ShouldSetCR1) {
	timer_set_clock_divider(PERIPH_TIM1, 5);
	LONGS_EQUAL(0, TIM1->CR1);
	timer_set_clock_divider(PERIPH_TIM1, 4);
	LONGS_EQUAL(0x200, TIM1->CR1);
	timer_set_clock_divider(PERIPH_TIM1, 2);
	LONGS_EQUAL(0x100, TIM1->CR1);
	timer_set_clock_divider(PERIPH_TIM1, 1);
	LONGS_EQUAL(0, TIM1->CR1);
}

TEST(Timer, set_counter_direction_ShouldSetCR1) {
	timer_set_counter_direction(PERIPH_TIM1, TIMER_DIRECTION_DOWN);
	LONGS_EQUAL(0x10, TIM1->CR1);
	timer_set_counter_direction(PERIPH_TIM1, TIMER_DIRECTION_UP);
	LONGS_EQUAL(0, TIM1->CR1);
}

TEST(Timer, set_counter_alignment_mode_ShouldSetCR1) {
	timer_set_counter_alignment_mode(PERIPH_TIM1, 1);
	LONGS_EQUAL(0x20, TIM1->CR1);
	timer_set_counter_alignment_mode(PERIPH_TIM1, 2);
	LONGS_EQUAL(0x40, TIM1->CR1);
}

TEST(Timer, set_cc_mode_ShouldSetCCMR_WhenToggleGiven) {
	timer_set_cc_pin_mode(PERIPH_TIM1, TIMER_CC_1, TIMER_CC_MODE_TOGGLE);
	LONGS_EQUAL(0x30, TIM1->CCMR1);
	timer_set_cc_pin_mode(PERIPH_TIM1, TIMER_CC_2, TIMER_CC_MODE_TOGGLE);
	LONGS_EQUAL(0x3030, TIM1->CCMR1);
	timer_set_cc_pin_mode(PERIPH_TIM1, TIMER_CC_3, TIMER_CC_MODE_TOGGLE);
	LONGS_EQUAL(0x30, TIM1->CCMR2);
	timer_set_cc_pin_mode(PERIPH_TIM1, TIMER_CC_4, TIMER_CC_MODE_TOGGLE);
	LONGS_EQUAL(0x3030, TIM1->CCMR2);
}
TEST(Timer, set_cc_mode_ShouldSetCCMR_WhenPwmActiveHighGiven) {
	timer_set_cc_pin_mode(PERIPH_TIM1, TIMER_CC_1,
			TIMER_CC_MODE_PWM_ACTIVE_HIGH);
	LONGS_EQUAL(0x60, TIM1->CCMR1);
	timer_set_cc_pin_mode(PERIPH_TIM1, TIMER_CC_2,
			TIMER_CC_MODE_PWM_ACTIVE_HIGH);
	LONGS_EQUAL(0x6060, TIM1->CCMR1);
	timer_set_cc_pin_mode(PERIPH_TIM1, TIMER_CC_3,
			TIMER_CC_MODE_PWM_ACTIVE_HIGH);
	LONGS_EQUAL(0x60, TIM1->CCMR2);
	timer_set_cc_pin_mode(PERIPH_TIM1, TIMER_CC_4,
			TIMER_CC_MODE_PWM_ACTIVE_HIGH);
	LONGS_EQUAL(0x6060, TIM1->CCMR2);
}

TEST(Timer, set_cc_pin_ShouldSetCCMR) {
	timer_set_cc_pin(PERIPH_TIM1, TIMER_CC_1, 1);
	LONGS_EQUAL(1, TIM1->CCMR1);
	timer_set_cc_pin(PERIPH_TIM1, TIMER_CC_2, 2);
	LONGS_EQUAL(0x201, TIM1->CCMR1);
	timer_set_cc_pin(PERIPH_TIM1, TIMER_CC_3, 3);
	LONGS_EQUAL(3, TIM1->CCMR2);
	timer_set_cc_pin(PERIPH_TIM1, TIMER_CC_4, 0);
	LONGS_EQUAL(3, TIM1->CCMR2);
}

TEST(Timer, enable_cc_pin_ShouldSetCCER) {
	timer_enable_cc_pin(PERIPH_TIM1, TIMER_CC_1);
	LONGS_EQUAL(1, TIM1->CCER);
	timer_enable_cc_pin(PERIPH_TIM1, TIMER_CC_1N);
	LONGS_EQUAL(5, TIM1->CCER);
	timer_enable_cc_pin(PERIPH_TIM1, TIMER_CC_3);
	LONGS_EQUAL(0x105, TIM1->CCER);
}
TEST(Timer, disable_cc_pin_ShouldClearCCER) {
	timer_enable_cc_pin(PERIPH_TIM1, TIMER_CC_3);
	timer_disable_cc_pin(PERIPH_TIM1, TIMER_CC_3);
	LONGS_EQUAL(0, TIM1->CCER);
}

TEST(Timer, set_cc_polarity_ShouldSetCCER) {
	timer_set_cc_pin_polarity(PERIPH_TIM1, TIMER_CC_1, 0);
	LONGS_EQUAL(2, TIM1->CCER);
	timer_set_cc_pin_polarity(PERIPH_TIM1, TIMER_CC_1, 1);
	LONGS_EQUAL(0, TIM1->CCER);
	timer_set_cc_pin_polarity(PERIPH_TIM1, TIMER_CC_2N, 0);
	LONGS_EQUAL(0x80, TIM1->CCER);
	timer_set_cc_pin_polarity(PERIPH_TIM1, TIMER_CC_3, 0);
	LONGS_EQUAL(0x280, TIM1->CCER);
}

TEST(Timer, enable_cc_preload_ShouldSetCCMR) {
	timer_enable_cc_preload(PERIPH_TIM1, TIMER_CC_1);
	LONGS_EQUAL(8, TIM1->CCMR1);
	timer_enable_cc_preload(PERIPH_TIM1, TIMER_CC_2);
	LONGS_EQUAL(0x808, TIM1->CCMR1);
	timer_enable_cc_preload(PERIPH_TIM1, TIMER_CC_3);
	LONGS_EQUAL(8, TIM1->CCMR2);
}
TEST(Timer, disable_cc_preload_ShouldClearCCMR) {
	TIM1->CCMR1 = TIM1->CCMR2 = 0xfff;
	timer_disable_cc_preload(PERIPH_TIM1, TIMER_CC_1);
	LONGS_EQUAL(0xff7, TIM1->CCMR1);
	timer_disable_cc_preload(PERIPH_TIM1, TIMER_CC_2);
	LONGS_EQUAL(0x7f7, TIM1->CCMR1);
	timer_disable_cc_preload(PERIPH_TIM1, TIMER_CC_3);
	LONGS_EQUAL(0xff7, TIM1->CCMR2);
}

TEST(Timer, enable_cc_fastmode_ShouldSetCCMR) {
	timer_enable_cc_fastmode(PERIPH_TIM1, TIMER_CC_1);
	LONGS_EQUAL(4, TIM1->CCMR1);
	timer_enable_cc_fastmode(PERIPH_TIM1, TIMER_CC_2);
	LONGS_EQUAL(0x404, TIM1->CCMR1);
	timer_enable_cc_fastmode(PERIPH_TIM1, TIMER_CC_3);
	LONGS_EQUAL(4, TIM1->CCMR2);
}
TEST(Timer, disable_cc_fastmode_ShouldClearCCMR) {
	TIM1->CCMR1 = TIM1->CCMR2 = 0xfff;
	timer_disable_cc_fastmode(PERIPH_TIM1, TIMER_CC_1);
	LONGS_EQUAL(0xffb, TIM1->CCMR1);
	timer_disable_cc_fastmode(PERIPH_TIM1, TIMER_CC_2);
	LONGS_EQUAL(0xbfb, TIM1->CCMR1);
	timer_disable_cc_fastmode(PERIPH_TIM1, TIMER_CC_3);
	LONGS_EQUAL(0xffb, TIM1->CCMR2);
}

TEST(Timer, set_cc_prescaler_ShouldSetCCMR) {
	timer_set_cc_prescaler(PERIPH_TIM1, TIMER_CC_1, 3);
	LONGS_EQUAL(0xc, TIM1->CCMR1);
	timer_set_cc_prescaler(PERIPH_TIM1, TIMER_CC_1, 2);
	LONGS_EQUAL(8, TIM1->CCMR1);
	timer_set_cc_prescaler(PERIPH_TIM1, TIMER_CC_4, 2);
	LONGS_EQUAL(0x800, TIM1->CCMR2);
}

TEST(Timer, set_cc_filter_ShouldSetCCMR) {
	timer_set_cc_filter(PERIPH_TIM1, TIMER_CC_1, 15);
	LONGS_EQUAL(0xf0, TIM1->CCMR1);
	timer_set_cc_filter(PERIPH_TIM1, TIMER_CC_1, 2);
	LONGS_EQUAL(0x20, TIM1->CCMR1);
	timer_set_cc_filter(PERIPH_TIM1, TIMER_CC_4, 2);
	LONGS_EQUAL(0x2000, TIM1->CCMR2);
}

TEST(Timer, set_slave_mode_ShouldSetSMCR) {
	timer_set_slave_mode(PERIPH_TIM1, 7);
	LONGS_EQUAL(7, TIM1->SMCR);
	timer_set_slave_mode(PERIPH_TIM1, 1);
