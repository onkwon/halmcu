...
static USART_Type *get_uart_from_port(periph_t port)
{
	switch (port) {
	case PERIPH_USART1:
		return USART1;
...
{
	return !!(get_uart_from_port(port)->SR & (1U << 5)/*RXNE*/);
}

bool uart_is_tx_ready(periph_t port)
{
	return !!(get_uart_from_port(port)->SR & (1U << 7)/*TXE*/);
}

int uart_get_rxd(periph_t port)
{
	return (int)get_uart_from_port(port)->DR;
}

void uart_set_txd(periph_t port, uint32_t value)
{
	get_uart_from_port(port)->DR = value;
}
...
void uart_set_baudrate(periph_t port, uint32_t baudrate, uint32_t pclk)
{
	...
	get_uart_from_port(port)->BRR = brr2reg(baudrate, pclk);
}

void uart_enable_irq(periph_t port, uart_irq_t irqs)
{
	if (irqs & UART_IRQ_RX) {
		bitop_set(&get_uart_from_port(port)->CR1, 5/*RXNEIE*/);
	}
	if (irqs & UART_IRQ_TX_READY) {
		bitop_set(&get_uart_from_port(port)->CR1, 7/*TXEIE*/);
	}
}
...
{
	if (irqs & UART_IRQ_RX) {
		bitop_clear(&get_uart_from_port(port)->CR1, 5/*RXNEIE*/);
	}
	if (irqs & UART_IRQ_TX_READY) {
		bitop_clear(&get_uart_from_port(port)->CR1, 7/*TXEIE*/);
	}
}

uart_event_t uart_get_event(periph_t port)
{
	uint32_t flags = get_uart_from_port(port)->SR;
	uint32_t events = 0;
...
void uart_set_parity(periph_t port, uart_parity_t parity)
{
	if (parity == UART_PARITY_NONE) {
		bitop_clear(&get_uart_from_port(port)->CR1, 10/*PCE*/);
		return;
	}
	...
	bitop_clean_set_with_mask(&get_uart_from_port(port)->CR1, 9, 3, val);
}
...
void uart_set_stopbits(periph_t port, uart_stopbit_t stopbit)
{
	...
	bitop_clean_set_with_mask(&get_uart_from_port(port)->CR2, 12, 3, val);
}

void uart_set_wordsize(periph_t port, uart_wordsize_t wordsize)
{
	assert(wordsize >= UART_WORDSIZE_8 && wordsize <= UART_WORDSIZE_9);
	bitop_clean_set_with_mask(&get_uart_from_port(port)->CR1,
			12, 1, wordsize & 1);
}

void uart_start(periph_t port)
{
	bitop_set(&get_uart_from_port(port)->CR1, 2/*RE*/);
	bitop_set(&get_uart_from_port(port)->CR1, 3/*TE*/);
	bitop_set(&get_uart_from_port(port)->CR1, 13/*UE*/);
}

void uart_stop(periph_t port)
{
	bitop_clear(&get_uart_from_port(port)->CR1, 2/*RE*/);
	bitop_clear(&get_uart_from_port(port)->CR1, 3/*TE*/);
	bitop_clear(&get_uart_from_port(port)->CR1, 13/*UE*/);
}
